...
フック名の事前検証は行いません。未対応のフック名は発火せず、別タイミングへのフォールバックもしません。
...
`agent_end` は最終テキスト回答のみが対象です。ツール呼び出し中・中止・失敗した回答は評価しません。同じ回答でも、イベントが再度発火すれば再評価します。
...
- その他のフック: フック名と型付き回答を通知するだけで、評価ログは保存しません。会話コンテキストや実行キューにも入れません。表示した判定が次のターンを呼び、再評価が続くループを防ぐためです。

これは**評価タイミングの指定**です。Piフックの戻り値を設定する機能ではなく、判定結果によるツールのブロック・入力変換・自動実行は行いません。
`/alter-ego` のOFF、UIのない実行では新たな評価を開始しません。ON/OFF設定の保存・ブランチからの復元は維持します。
判断はJevのみで行います。

## 実行フロー

設定読込 → 質問選択 → 評価用state構築 → Jevへ1回送信 → 回答表示、の直線的なフローです。

- Resultへの変換、独自のエラー通知、再試行、タイムアウトはありません。
- 評価済み状態や処理中リクエストを追跡せず、重複排除・鮮度チェック・古い結果の破棄も行いません。
- Piから渡されたAbortSignalはfetchへそのまま渡します。セッション移動やON/OFF切替による独自のキャンセル処理はありません。
- 設定・応答の追加スキーマ検証は行わず、設定形式とTypeSafeのAPI契約を前提にします。
- 通信・HTTP・JSON解析などの失敗は呼び出し元へ伝播します。Pi側の標準的な例外処理が適用されるため、例えば`tool_call`の評価失敗はPi側でツールをブロックする場合があります。
