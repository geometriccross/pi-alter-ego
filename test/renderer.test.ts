...
const message = { role: "custom", customType: "alter-ego", display: true, timestamp: 0, content: "Jev judgment\nsource: **verbatim** [quote](https://example.invalid)", details: { assessment: { version: 1, model: "jev-test", probability: 0.97 } } } as const;
...
describe("Jev message renderer", () => {
  ...
  it("makes raw judgments available when expanded", () => {
    ...
    expect(output).toContain('"probability": 0.97');
  ...
  it("does not relabel historical generated messages as Jev, or execute terminal controls in details", () => {
    const legacy = { ...message, content: "old response", details: { quote: "\u009b2J\u001b[2J" } };
  ...
  it("renders persisted text blocks too", () => {
    const output = renderAlterEgoMessage({ ...message, content: [{ type: "text", text: "stored" }] }, { expanded: false }, theme)!.render(80).join("\n");
